...
function sendData() {
	...
	if (!hasErrors) {
		...
		stompClient.subscribe('/topic/return-to', function(response) {
			...
			if (returnMessage.error) {
				// Display the error message on the form
				//messageShow.textContent = returnMessage.error;
...
function updateTable(allUser) {
	...
	allUser.forEach((user) => {
		...
		row.innerHTML = `
            ...
            <td>${user.status}</td>
            <td>
                 <button onclick="openEditForm(${user.id},'${user.idTag}','${user.username}','${user.expiryDate}','${user.parentIdTag}','${user.status}')" style="border: none; color:blue;" ><span id="updateButton" style="color:blue;">&#9998;</span></button>
...
function openEditForm(userId , idtag , userName , expiryDate , parentidTag , Status)
 {
